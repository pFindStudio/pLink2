...
BMIonGenerator::BMIonGenerator(const SearchParameter *pParameter) :
...
const string BMIonGenerator::strTitlePrefix("Simulation");
const string BMIonGenerator::strTitleSuffix("dta");
//size_t BMIonGenerator::m_tScanNo = 1000;
size_t BMIonGenerator::m_tScanNo = 1; // scan从1开始编号

BMIonGenerator::~BMIonGenerator()
{
	if(m_pBias)
		delete m_pBias;
	m_pBias = NULL;
...
size_t BMIonGenerator::generateCharge()
{
...
void BMIonGenerator::insertRatio(map<pair<char, int>, double> &mpStatistic, char cType, int nCharge, double lfRatio) {
	mpStatistic.insert(pair<pair<char, int>, double>(pair<char, int>(cType, nCharge), lfRatio));
}

void BMIonGenerator::insertRatio(map<pair<char, int>, bool> &mpStatistic, char cType, int nCharge, double lfRatio) {
	double lfTotal = 1141.0;
	double lfRandom = (double)rand() / RAND_MAX;
	mpStatistic.insert(pair<pair<char, int>, bool>(pair<char, int>(cType, nCharge), lfRandom < lfRatio/lfTotal));
}

void BMIonGenerator::genMatchRatio()
{
...
void BMIonGenerator::genIntensityMean()
{
...
void BMIonGenerator::genExistFlag()
{
...
double BMIonGenerator::getMatchRatio(IonType stIonType)
{
...
double BMIonGenerator::getIntensityMean(IonType stIonType)
{
	double lfRatio = 0.03;
...
bool BMIonGenerator::getExistFlag(IonType stIonType)
{
	bool bFlag = true;
...
void BMIonGenerator::mergePeaks()
{
...
double BMIonGenerator::getPepTypeScore(PeptideType ePepType){
	double arrExtraScore[] = {0.02, 0.02, 0.03, 0.001};
	return arrExtraScore[ePepType];
}

void BMIonGenerator::match()
{
	...
	for(size_t tPepPos = 0; tPepPos < m_vIonMz.size(); ++tPepPos) {
		...
		if(rand()%4 == 0){
			...
			stIonPeak.m_lfIntensity = INTENSITY_MULTIPLIER * 0.03;
